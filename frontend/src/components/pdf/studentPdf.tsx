...
export function generateStudentPDF(data: any) {
  const doc = new jsPDF();
  ...
  y = addTextBlock(doc, "Student / Internship Agreement Summary", 14, y, {});
  y += 2;
  ...
  if (Array.isArray(data.clauses) && data.clauses.length > 0) {
    ...
    y = addTextBlock(doc, "Important Clauses", 14, y, {});
    y += 2;
  ...
  // Key Legal Notes
  if (Array.isArray(data.keyLegalNotes) && data.keyLegalNotes.length > 0) {
    doc.setFontSize(13).setTextColor(34, 64, 123).setFont("helvetica", "bold");
    y = addTextBlock(doc, "Key Legal Notes", 14, y, {});
    y += 2;
    doc.setFontSize(11).setTextColor(40, 40, 40).setFont("helvetica", "normal");
    data.keyLegalNotes.forEach((note: string) => {
  ...
  // Final Tips
  if (Array.isArray(data.finalTips) && data.finalTips.length > 0) {
    doc.setFontSize(13).setTextColor(34, 64, 123).setFont("helvetica", "bold");
    y = addTextBlock(doc, "Final Tips", 14, y, {});
    y += 2;
    doc.setFontSize(11).setTextColor(40, 40, 40).setFont("helvetica", "normal");
    data.finalTips.forEach((tip: string) => {
      y = addTextBlock(doc, `• ${tip}`, 18, y, {});
    });
    y += 6;
  }
